const { app, BrowserWindow, ipcMain, webContents, clipboard, Menu, dialog, shell } = require('electron');
...
function findFileRecursively(startDir, targetName) {
...
async function withArduinoAction(startMessage, successMessage, action) {
...
function readUcBlocklyPackageMetadata() {
  const candidates = [
...
async function checkAndInstallAppUpdate() {
  ...
  try {
    const release = await fetchJson(APP_RELEASES_API_URL);
...
function getUcBlocklyIndexUrl(localeCode) {
...
function readUcBlocklyInstalledSha() {
  try {
...
function writeUcBlocklyVersion(sha) {
  const versionPath = getUcBlocklyVersionPath();
  const payload = {
    sha,
...
async function fetchLatestUcBlocklySha() {
  const result = await fetchJson(UCBLOCKLY_COMMIT_API_URL);
  const sha = result && typeof result.sha === 'string' ? result.sha.trim() : '';
  if (!sha) throw new Error(t().ucBlocklyNoCommitSha);
  return sha;
}

async function installUcBlocklyBundle(sha) {
...
function loadLocaleFileFromPath(localePath) {
...
function normalizeLocaleCode(rawCode) {
...
async function executeScriptInWokwi(script) {
...
async function injectCodeIntoWokwi(code) {
  ...
  const result = await executeScriptInWokwi(script);
  return !!(result && result.ok);
}

async function extractCodeFromWokwi() {
  const script = `
    (() => {
      const monacoRef = window.monaco;
  ...
  const extracted = await executeScriptInWokwi(script);
  if (!extracted || extracted === EMPTY_CODE) return '';
  return extracted;
}

function findWokwiWebContents() {
...
function showRendererNotification(message) {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  mainWindow.webContents.send('menu-notification', message);
}

function showArduinoCliPanel(message, active) {
  if (!mainWindow || mainWindow.isDestroyed()) return;
...
function appendArduinoCliConsole(text) {
  if (!mainWindow || mainWindow.isDestroyed()) return;
...
function sanitizeLibraryName(rawName) {
  if (!rawName || typeof rawName !== 'string') return '';
  return rawName.trim().replace(/[^\w\s.\-/:]/g, '');
}

async function installArduinoLibraryFromDialog(rawLibraryName, sender) {
...
function createWindow() {
  ...
  ipcMain.on('generated-code-updated', (_event, code) => {
    latestGeneratedCode = typeof code === 'string' ? code : '';
  });

  ipcMain.removeHandler('inject-code-wokwi');
  ipcMain.handle('inject-code-wokwi', async (_event, code) => {
  ...
  ipcMain.handle('get-ucblockly-url', async (_event, localeCode) => {
